...
describe("AgentMerchant", function () {
  ...
  describe("fullfillSellStock", function () {
    ...
    it("should pay back USDC to users who sold tokens", async function () {
      ...
      // Since there's no direct way to get the length, we'll check if accessing the first element fails or returns empty
      const sellRequests = await agentMerchant.sellShareRequests(agentTokenAddress, 0);
      expect(sellRequests.userWalletAddress).to.equal(ethers.constants.AddressZero);
      expect(sellRequests.tokenAmount).to.equal(0);
    });
